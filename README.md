...
SQL Auto Importis a command-line tool, written in Python®, that allows you to quickly create/populate or replace tables in a SQL database with CSV files.
...
# Usage
The source Python file (.py) is provided, which can be used on any system where Python is installed. For Windows users who do not wish to use or install Python, a [64-bit executable file (.exe)](https://github.com/kevinlakhani/sqlautoimport/raw/master/sqlautoimport.exe) is provided in the /dist/ folder ([or just download it here](https://github.com/kevinlakhani/sqlautoimport/raw/master/sqlautoimport.exe)).
